...
RequestHandler::RequestHandler( TransportCatalogue::TransportCatalogue& trnsprt_ctlg,
    ...
    :trnsprt_ctlg_(trnsprt_ctlg)
    ,trnsprt_routr_(trnsprt_routr_)
    ,map_rendr_(map_rendr_)
{
...
std::optional<domain::RoutStat> RequestHandler::GetRouteStat(std::string_view stop_from, std::string_view stop_to) const
{
    ...
    return trnsprt_routr_.GetRouteStat(trnsprt_ctlg_.FindStop(stop_from).value()->id, trnsprt_ctlg_.FindStop(stop_to).value()->id);
}
//----------------------------------------------------------------------------
